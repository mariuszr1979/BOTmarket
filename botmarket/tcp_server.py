...
def handle_register_schema(payload: bytes) -> bytes:
    # payload: [api_key_len:2][api_key][json_body]
    import struct
    key_len = struct.unpack('!H', payload[:2])[0]
    api_key = payload[2:2 + key_len].decode()
    body = json.loads(payload[2 + key_len:])

    agent_pubkey = _auth_key(api_key.encode())
    if not agent_pubkey:
        return pack_error(0x02, b"invalid api key")
...
def handle_match(payload: bytes) -> bytes:
    ...
    buyer_pubkey = _auth_key(api_key.encode())
    if not buyer_pubkey:
        return pack_error(0x02, b"invalid api key")
...
def handle_execute(payload: bytes) -> bytes:
    ...
    caller_pubkey = _auth_key(api_key.encode())
